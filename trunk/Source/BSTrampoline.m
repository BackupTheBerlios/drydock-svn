...
+ (NSMethodSignature *)signatureWithObjCTypes:(const char *)inTypes;
...
- (id)initWithMethodSignature:(NSMethodSignature *)inSignature;
...
- (id)initWithEnumerator:(NSEnumerator *)inEnumerator countingTo:(unsigned *)outCount {
	if (NULL == outCount)
		[NSException raise:@"InvalidArgumentException" format:@"outCount argument of initWithEnumerator:countingTo: was NULL."];
	enumerator = [inEnumerator retain];
	mode = kCountMode;
	counter = outCount;
	*counter = 0;
